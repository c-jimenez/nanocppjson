...
TEST_SUITE("json parsing test suite")
{
    ...
    TEST_CASE("String value")
    {
        ...
        char             tmp1[53u];
        std::span<char>  tmp_span1        = tmp1;
        std::string_view string3_escaped1 = string3->get_escaped(tmp_span1);
        CHECK(string3_escaped1.empty());

        char             tmp2[54u];
        std::span<char>  tmp_span2        = tmp2;
        std::string_view string3_escaped2 = string3->get_escaped(tmp_span2);
    ...
    TEST_CASE("Decimal value")
    {
        ...
        CHECK_EQ(dec1->get<double>(), 12345.6789);
        ...
        CHECK_EQ(dec2->get<double>(), -12345.6789);
    ...
    TEST_CASE("Array value")
    {
        ...
        static const std::array<nanojsoncpp::json_value_type, 4u> expected_float_types  = {nanojsoncpp::json_value_type::integer,
                                                                                           ...
                                                                                           nanojsoncpp::json_value_type::integer,
                                                                                           nanojsoncpp::json_value_type::decimal};
        static const std::array<double, 4u>                       expected_float_values = {123456789., -1234.5678, -1234567890, 12345.678};
        index                                                                           = 0u;
        for (const auto& value : root4.value())
        {
            CHECK_EQ(value.type(), expected_float_types[index]);
            CHECK_EQ(value.name(), "");
            CHECK_EQ(value.get<double>(), expected_float_values[index]);
            index++;
        }
