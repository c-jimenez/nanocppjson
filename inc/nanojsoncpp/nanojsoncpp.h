 ...
 */
using json_size_t = uint16_t;

// Check configuration
static_assert(std::is_integral_v<json_size_t> && std::is_unsigned_v<json_size_t>);
...
{
  public:
    /** @brief Default constructor, needed for array and optional => Do not use!! */
    ...
    std::string_view get_escaped(std::span<char>& buffer) const
    {
    ...
    optional<json_value> get(const std::string_view& name) const
    {
        optional<json_value> value;
    ...
    optional<json_value> get(json_size_t index) const
    {
        optional<json_value> value;
    ...
    /** @brief Get a member value (only for object types) */
    optional<json_value> operator[](const std::string_view& name) const { return get(name); }

    /** @brief Get an array value (only for array types) */
    optional<json_value> operator[](json_size_t index) const { return get(index); }
