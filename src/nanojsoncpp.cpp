...
static optional<char> get_next_not_blank_char(const std::string_view& str, json_size_t& index);

/** @brief Move to the next char in the input string which is not in the provided char list */
static optional<char> get_next_char_not_in(const std::span<const char>& not_in_chars, const std::string_view& str, json_size_t& index);

/** @brief Move to the next expected char in the input string */
static optional<char> get_next_char(const std::span<const char>& expected_chars, const std::string_view& str, json_size_t& index);

/** @brief Get a string value on the input string */
static optional<std::string_view> get_string_value(parse_context& ctxt, const std::string_view& str);
...
optional<json_value> parse(const std::string_view&           json_string,
                           const std::span<json_value_desc>& descriptors,
                           parse_error_handler_t             error_handler)
{
    optional<json_value> root;
    ...
    if (json_string.size() <= MAX_JSON_STRING_SIZE)
    {
        ...
        if (!descriptors.empty())
        {
            ...
            optional<char> root_start_c = get_next_not_blank_char(json_string, start_index);
            if (root_start_c)
            {
                ...
                if (success && (ctxt.index < json_string.size()))
                {
                    ...
                    optional<char> c          = get_next_not_blank_char(json_string.substr(ctxt.index), next_index);
                    if (c)
                    {
                ...
                if (success)
                {
                    ...
                    root.set_value(json_value(json_string, root_value_desc));
...
static optional<std::string_view> get_string_value(parse_context& ctxt, const std::string_view& str)
{
    optional<std::string_view> str_value;
    ...
    while (!end_of_str)
    {
        ...
        optional<char> c       = get_next_char(STR_NEXT_CHAR, sv, c_index);
        if (c)
        {
            str_index += c_index;
            if (*c == '"')
            {
                // End of string
                str_value.set_value(str.substr(0u, str_index));
                str_index++;
                end_of_str = true;
            }
            else
...
static bool parse_start_of_member_name(parse_context& ctxt, const std::string_view& json_string)
{
    ...
    json_size_t    next_index = 0u;
    optional<char> next       = get_next_not_blank_char(json_string.substr(ctxt.index), next_index);
...
static bool parse_member_name(parse_context& ctxt, const std::string_view& json_string)
{
    ...
    optional<std::string_view> name        = get_string_value(ctxt, json_string.substr(ctxt.index));
    if (name)
    {
        // Save position
...
static bool parse_string_value(parse_context& ctxt, const std::string_view& json_string)
{
    ...
    if (start_of_string < json_string.size())
    {
        ...
        optional<std::string_view> value = get_string_value(ctxt, json_string.substr(start_of_string));
        if (value)
        {
            // Save position
...
static bool parse_decimal_value(parse_context& ctxt, const std::string_view& json_string)
{
    ...
    json_size_t    next_index = 0u;
    optional<char> next       = get_next_char_not_in(DECIMAL_CHARS, json_string.substr(ctxt.index), next_index);
    if (next)
    {
        // Extract value
