...
[![CodeQL Advanced](https://github.com/c-jimenez/nanocppjson/actions/workflows/codeql-analysis.yml/badge.svg?branch=main)](https://github.com/c-jimenez/nanocppjson/actions/workflows/codeql-analysis.yml)
[![Unit tests - clang](https://github.com/c-jimenez/nanocppjson/actions/workflows/unit-test-clang.yml/badge.svg?branch=main)](https://github.com/c-jimenez/nanocppjson/actions/workflows/unit-test-clang.yml)
[![Unit tests - gcc](https://github.com/c-jimenez/nanocppjson/actions/workflows/unit-test-gcc.yml/badge.svg?branch=main)](https://github.com/c-jimenez/nanocppjson/actions/workflows/unit-test-gcc.yml)
...
* Maximum json input string size is 65535 bytes
...
* Decimals: only fractional representation is supported, not exponent representation (ex: 3.1415926 is supported, 3.141593e+00 is not supported)
* Integers: only the values in the `int64_t` range can be represented (ie. from -9223372036854775808 to 9223372036854775807)
...
The `json_val` variable is of type `nanojsoncpp::optional<nanojsoncpp::json_value>`. The `nanojsoncpp::optional` as a behavior similar to the one of `std::optional` except that it doesn't relies on dynamic memory allocation.
...
{
    char tmp[100u];
    std::span<char, sizeof(tmp)> s_tmp;
    std::string_view value = json_val.get_escaped(s_tmp);
    // Use value
}
```

**Note 1:** The returned `std::string_view` object uses as its underlying buffer the `tmp` variable provided through the `s_tmp` `std::span` object. If the `tmp` variable is not large enough to store the escaped string, the returned value will be empty. The provided buffer size must have at least the size of the raw string. 
